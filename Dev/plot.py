...
def plot_data(data, data_depth, title, save_plot, show_plot=True):
    ...
    #find first and last points of the linear regression
    def get_points(x, y):
        a,b=lin_reg(x,y)
    ...
    #build plot from the data matrix with the linear regression
    def build_plot(data_matrix, title, save_plot, extra_text="", fig_numb=1):
        for elem in data_matrix:
            ...
            plt.title(extra_text+str(elem)+str(title)[:-2])
            color=0
