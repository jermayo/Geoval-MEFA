...
def diff_time(raw_datas, delta_t, delta_temp, time_max_event, month_start, month_end):
    ...
    datas=diff_temp(raw_datas, delta_t)

    for data in datas:
        year=data["f_data"]["date"].year
        if year not in year_list:
            year_list.append(year)
            new_year={"year": year, "pos":0, "neg":0, "total":0}
            event_list.append(new_year)

        if not during_event and abs(data["dtemp"])>delta_temp:
            if date_beetween(data["f_data"]["date"], month_start, month_end):
                during_event=True
                event_type="+"
                if data["dtemp"]<0:
                    event_type="-"
            ...
            if data["f_data"]["date"]-new_event["start"]>time_max_event:
                during_event=False
                for event_year in event_list:
                    if event_year["year"]==new_event["start"].year:
                            if new_event["type"]=="+":
                                event_year["pos"]+=1
                            elif new_event["type"]=="-":
                                event_year["neg"]+=1
...
class Window():
    ...
    def analyse(self):
        ...
        if analyse=="Difference Time":
            ...
            res_total=[]
            if period==1:
                period_list=[[1,13]] #period list from month# to month# NOT INCLUDED 13 => december included
                res_text="Year\tPos.\tNeg.\tTotal\n"
            elif period==2:
                period_list=[[12,3],[3,6],[6,8],[9,12]]
                res_text="Year\tWinter\t\t\tSpring\t\t\tSummer\t\t\tAutumn\n"
                res_text+="\tPos.\tNeg.\tTotal"*4
                res_text+="\n"
            elif period==3:
                period_list=[]
                res_text="Year\t"
                for i in range(1,13):
                    period_list.append([i, i+1])
                    res_text+=month_abbr[i]+"\t\t\t"
                res_text+="\n"
                res_text+="\tPos.\tNeg.\tTotal"*12
                res_text+="\n"
            for i in period_list:
                res=diff_time(GV.datas, delta_t, int(self.Sb_tweak1.get()), time_max_event, i[0], i[1])
    ...
    def change_analyse(self, value):
        ...
        if value=="Difference Time":
            ...
            tk.Label(self.tweak_frame, text="Period:").grid(row=1, column=4)
            tk.Radiobutton(self.tweak_frame, var=self.period, text="All Year", value=1).grid(row=2, column=4, sticky=tk.W)
            tk.Radiobutton(self.tweak_frame, var=self.period, text="Season", value=2).grid(row=3, column=4, sticky=tk.W)
            tk.Radiobutton(self.tweak_frame, var=self.period, text="Month", value=3).grid(row=4, column=4, sticky=tk.W)
    ...
    def load_begin(self):
        ...
        tk.Label(self.load_w, text="Loading, please wait...").grid(row=1,column=1)
        tk.Button(self.load_w, text="Cancel", state="disable").grid(row=2, column=1)
